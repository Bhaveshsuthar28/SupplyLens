...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => MOCK_SUPPLIERS.filter((s) => {
    if (category !== "All Categories" && s.category !== category) return false;
    if (grade !== "All Grades" && s.grade !== grade) return false;
    return true;
  }), [category, grade]);

  const avgOTD = (filtered.reduce((a, s) => a + s.otd, 0) / filtered.length).toFixed(1);
  const avgFR = (filtered.reduce((a, s) => a + s.fillRate, 0) / filtered.length).toFixed(1);
  const avgReject = (filtered.reduce((a, s) => a + s.rejectRate, 0) / filtered.length).toFixed(1);
            ...
            <ChartCard title="Rejection Rate Distribution">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={rejectionDist}>
                  ...
                  <Tooltip contentStyle={{ background: "hsl(210,11%,15%)", border: "1px solid hsl(210,11%,25%)", color: "#fff", fontFamily: "'IBM Plex Mono'", fontSize: 11 }} />
            ...
            <ChartCard title="Top Suppliers vs Rank">
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={topSuppliersTrend}>
                  ...
                  <YAxis domain={["auto", "auto"]} tick={chartStyle} stroke="hsl(210,14%,50%)" />
                  <Tooltip contentStyle={{ background: "hsl(210,11%,15%)", border: "1px solid hsl(210,11%,25%)", color: "#fff", fontFamily: "'IBM Plex Mono'", fontSize: 11 }}
                    formatter={(value: number, _: string, props: any) => [`${value} (${props.payload.supplier})`, "Score"]} />
            ...
            <ChartCard title="Supplier Risk Matrix">
              <ResponsiveContainer width="100%" height={220}>
                <ScatterChart>
                  ...
                  <Tooltip contentStyle={{ background: "hsl(210,11%,15%)", border: "1px solid hsl(210,11%,25%)", color: "#fff", fontFamily: "'IBM Plex Mono'", fontSize: 11 }}
                    formatter={(value: number, name: string) => [name === "OTD %" ? `${value}%` : value, name]}
...
function KPIBox({ icon, label, value, valueColor = "text-sidebar-foreground" }: { icon: React.ReactNode; label: string; value: string; valueColor?: string }) {
  return (
